...
class JassIntentionHexToRawCode : IntentionAction {
    ...
    private fun run(editor: Editor?, file: PsiFile?, invoke: Boolean): Boolean {
        ...
        if (element.elementType == HEXVAL) {
            val raw = RawCode.fromHexString(element.text, RawCodeLanguage.JASS)
            if (!invoke) return raw.safe && raw.valid
            if (ApplicationManager.getApplication().isWriteAccessAllowed) {
                JassElementTextFactory.replacePrimText(element, raw.quoted)
            }
