    ...
    protected open val spacingBuilder: SpacingBuilder?
        get() {
            ...
            return SpacingBuilder(myCodeStyleSettings, instance)
                .after(JassTypes.COMMA).spacing(sac, sac, 0, false, 0)
                .before(JassTypes.COMMA).spacing(sbc, sbc, 0, false, 0)
                .around(JassTypes.EQ).spacing(saao, saao, 0, false, 0)
                .between(JassTypes.TYPE_NAME, JassTypes.ID).spacing(1, 1, 0, false, 0)
                .around(JassTypes.NATIVE).spacing(1, 1, 0, false, 0)
                .around(JassTypes.FUN_TAKE).spacing(1, 1, 0, false, 0)
                .around(JassTypes.TAKES).spacing(1, 1, 0, false, 0)
                .after(JassTypes.RETURNS).spacing(1, 1, 0, false, 0)
        }
