package guru.xgm.jgas.lang.folding;
...
final class FoldingBuilderEx_JGAS extends FoldingBuilderEx implements DumbAware {

  @Override
  public FoldingDescriptor @NotNull [] buildFoldRegions(@NotNull PsiElement root, @NotNull Document document, boolean quick) {
    // Initialize the group of folding regions that will expand/collapse together.
    FoldingGroup group = FoldingGroup.newGroup(Annotator_JGAS.SIMPLE_PREFIX_STR);
    ...
    root.accept(new JavaRecursiveElementWalkingVisitor() {
      ...
      public void visitLiteralExpression(@NotNull PsiLiteralExpression literalExpression) {
        ...
        if (value != null && value.startsWith(Annotator_JGAS.SIMPLE_PREFIX_STR + Annotator_JGAS.SIMPLE_SEPARATOR_STR)) {
          ...
          JGAS_Property simpleProperty = ContainerUtil.getOnlyItem(Util_JGAS.findProperties(project, key));
  ...
  public String getPlaceholderText(@NotNull ASTNode node) {
    if (node.getPsi() instanceof PsiLiteralExpression psiLiteralExpression) {
      ...
      String key = text.substring(Annotator_JGAS.SIMPLE_PREFIX_STR.length() +
          Annotator_JGAS.SIMPLE_SEPARATOR_STR.length());

      JGAS_Property simpleProperty = ContainerUtil.getOnlyItem(
          Util_JGAS.findProperties(psiLiteralExpression.getProject(), key)
      );
      if (simpleProperty == null) {
