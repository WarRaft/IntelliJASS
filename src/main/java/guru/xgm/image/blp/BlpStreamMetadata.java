...
public final class BlpStreamMetadata extends IIOMetadata {
     ...
     * @throws IllegalArgumentException if encodingType is UNKNOWN.
     ...
     * @throws IllegalArgumentException if version is not a supported version (0 to 2).
     */
    ...
    public void readObject(ImageInputStream src) throws IOException {
        ...
        if (content >= ctvalues.length || content < 0) {
            ...
            warning.accept(BlpBundle.message("BadContent", content, defaultContentType.name())
            );
